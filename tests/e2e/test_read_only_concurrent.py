...
async def test_concurrent_readonly_connections(test_db_path):
    ...
    async with client1, client2:
        ...
        result1 = await client1.call_tool_mcp("query", {"query": "SELECT COUNT(*) as cnt FROM users"})
        result2 = await client2.call_tool_mcp("query", {"query": "SELECT COUNT(*) as cnt FROM movies"})
...
async def test_concurrent_readonly_parallel_queries(test_db_path):
    ...
    async with client1, client2, client3:
        ...
        results = await asyncio.gather(
            client1.call_tool_mcp("query", {"query": "SELECT * FROM users ORDER BY id LIMIT 1"}),
            client2.call_tool_mcp("query", {"query": "SELECT * FROM users ORDER BY id LIMIT 1"}),
            client3.call_tool_mcp("query", {"query": "SELECT * FROM users ORDER BY id LIMIT 1"}),
        )
...
async def test_readonly_does_not_block_other_readonly(test_db_path):
    ...
    async with client1:
        ...
        result1 = await client1.call_tool_mcp("query", {"query": "SELECT 1 as num"})
        assert result1.isError is False
        ...
        async with client2:
            ...
            result2 = await client2.call_tool_mcp("query", {"query": "SELECT 2 as num"})
            assert result2.isError is False
