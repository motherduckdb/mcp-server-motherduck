...
async def test_simple_select(motherduck_client):
    ...
    result = await motherduck_client.call_tool_mcp("query", {"query": "SELECT 1 as num"})
...
async def test_query_sample_data(motherduck_client):
    ...
    result = await motherduck_client.call_tool_mcp("query", {
...
async def test_list_databases(motherduck_client):
    ...
    result = await motherduck_client.call_tool_mcp("query", {
        "query": "SELECT database_name FROM duckdb_databases()"
    })
    assert result.isError is False
    text = get_result_text(result)
...
async def test_create_table_in_my_db(motherduck_client):
    ...
    result = await motherduck_client.call_tool_mcp("query", {
        "query": f"CREATE TABLE IF NOT EXISTS {table_name} (id INT, data VARCHAR)"
    })
    assert result.isError is False
    
    # Clean up
    await motherduck_client.call_tool_mcp("query", {
