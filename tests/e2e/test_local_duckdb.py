...
async def test_simple_select(local_client):
    ...
    result = await local_client.call_tool_mcp("query", {"sql": "SELECT 1 as num"})
...
async def test_query_users_table(local_client):
    ...
    result = await local_client.call_tool_mcp("query", {"sql": "SELECT * FROM users ORDER BY id"})
    assert result.isError is False
    text = get_result_text(result)
...
async def test_create_table(local_client):
    ...
    result = await local_client.call_tool_mcp(
        "query", {"sql": "CREATE TABLE IF NOT EXISTS test_temp (id INT, value VARCHAR)"}
    )
    assert result.isError is False

    # Verify table exists
    result = await local_client.call_tool_mcp("query", {"sql": "SELECT * FROM test_temp"})
    assert result.isError is False
...
async def test_insert_data(local_client):
    ...
    await local_client.call_tool_mcp(
        "query", {"sql": "CREATE TABLE IF NOT EXISTS test_insert (id INT, name VARCHAR)"}
    )

    # Insert data
    result = await local_client.call_tool_mcp(
        "query", {"sql": "INSERT INTO test_insert VALUES (1, 'Test')"}
    )
    assert result.isError is False

    # Verify data was inserted
    result = await local_client.call_tool_mcp(
        "query", {"sql": "SELECT * FROM test_insert WHERE id = 1"}
    )
    assert result.isError is False
    text = get_result_text(result)
    assert "Test" in text
...
async def test_aggregate_query(local_client):
    ...
    result = await local_client.call_tool_mcp(
        "query", {"sql": "SELECT COUNT(*) as total, AVG(id) as avg_id FROM large_table"}
    )
    assert result.isError is False
    text = get_result_text(result)
...
async def test_syntax_error_returns_error(local_client):
    ...
    result = await local_client.call_tool_mcp("query", {"sql": "SELEKT * FORM users"})
    assert result.isError is True
    text = get_result_text(result)
