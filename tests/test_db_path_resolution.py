...
def _set_env_token(monkeypatch) -> None:
    monkeypatch.delenv("motherduck_token", raising=False)
    monkeypatch.setenv("MOTHERDUCK_TOKEN", "env-token")


def test_env_token_saas_mode_appends_flag(monkeypatch):
    _set_env_token(monkeypatch)
    path, db_type = _client_without_init()._resolve_db_path_type("md:", saas_mode=True)
...
def test_env_token_without_saas_mode_does_not_append_flag(monkeypatch):
    _set_env_token(monkeypatch)
    path, db_type = _client_without_init()._resolve_db_path_type("md:", saas_mode=False)
    assert db_type == "motherduck"
