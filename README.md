...
| `--stateless-http` | `False` | For platform compatibility (e.g. [AWS Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-mcp-protocol-contract.html)). Does not change server state; the server still maintains global state via the shared DatabaseClient. |
